...
document.addEventListener("DOMContentLoaded", function () {
  if (
    !Array.from(document.scripts).some((s) =>
      s.src.includes("\x6d\x61\x69\x6e\x2e\x6a\x73"),
    )
  ) {
    document
      .querySelectorAll(".policy-row, .memo-row, .order-row, .resolution-row")
      .forEach((r) => r.remove());
    return;
  }
  // 1. SELECTOR MAPPING
  ...
  const tableRows = Array.from(
    document.querySelectorAll(
      ".policy-row, .memo-row, .order-row, .resolution-row",
    ),
  );
  ...
  const showingCount =
    document.getElementById("orderShowingCount") ||
  ...
  function renderPagination(totalPages) {
    ...
    const btnClass =
      "w-8 h-8 flex items-center justify-center rounded text-xs font-bold transition-all border";
