...
def push_to_db(post_data: Dict[str, Any]) -> bool:
    """Sync a single post to Supabase 'news' table."""
    if not supabase:
        return False
    
    # Map scraper format to DB schema
    title = post_data.get("caption", "").split("\n")[0][:100]
    if not title:
        title = "News Update"
    ...
    try:
        supabase.table("news").upsert(db_entry).execute()
        return True
                    ...
                    try:
                        ...
                        if caption_id and caption_id in existing_posts:
                            ...
                            if post_url and post_url in url_to_index:
                                ...
                                if not existing_entry.get("photos") and cached_photos:
                                    ...
                                    push_to_db(existing_entry) # Re-sync with images
                                ...
                                push_to_db(updated) # Sync to Supabase
                        ...
                        if caption or processed_photos:
                            ...
                            if post_url and post_url in url_to_index:
                                ...
                                push_to_db(posts_data[-1]) # Sync to Supabase
